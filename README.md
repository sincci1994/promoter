# 프로모터 플랫폼 서비스 기획서
> Version 0.2 (2026-08-10) — worker_interview 수용: 세전 표시 원칙(§9)·행사 특성 프리미엄(§9.3)·옵트인 연락 메모(§14.1)·행사 방(§14.2·§17)·Promoter 간 평가(§19.6)·온도 추진(§20)·§28 결정 항목 3건 추가  
> Version 0.1  
...
모든 단가 표시는 **세전 금액**이며, 사업소득 원천징수(3.3%) 후 지급을 전제한다(worker_interview의 지급 관행). 플랫폼 경유 지급 시 원천징수 처리 주체(지급대행 구조)는 §28에서 결정한다.
...
- Long Hours
- Outdoor (야외)
- Harsh Condition (열악 환경)
- Special Costume
- Training
- Rehearsal

프리미엄이 걸리는 행사 특성 조건(야외 여부, 교대 수 등)은 자가신고 태그가 아니라 구조화된 필드로 입력한다 — 태그 매칭으로 프리미엄이 새는 것을 차단하는 [D4](docs/domain/01-boundary-decisions.md) 결정 3과 같은 논리다. 상세 규칙은 Phase 1 Pricing Rule Table에서.
...
단, Worker가 원하면 인스타그램·연락처를 **옵트인 메모 필드**로 등록할 수 있다(worker_interview 수용 — 기본값은 미등록·비공개). 노출 범위는 `[정책]`(§28). 권장 기본값: 인스타그램 = 옵트인 시 프로필 공개(정보 차단이 아니라 §14.4의 경제 유인으로 직거래를 막는 원칙과 양립), 전화번호 = 등록해도 확정된 거래 상대에게만.
...
- **행사 방(Event Room) — 채택**: Booking 확정 시 자동 개설되는 행사 전용 방(참여자 = Buyer·Leader·확정 Worker). 공지와 양방향 소통을 담당한다(§17)
- In-App 1:1 Chat — 후보
- Relay Call — 후보
- 안심번호 — 후보
- Leader 중심 연락 — 후보
...
기본 채널은 **행사 방(Event Room)**이다 — Booking 확정 시 자동 개설되며(§14.2), 기존 관행의 "행사 전 단톡방"을 플랫폼 안으로 가져온 것이다(worker_interview). 아래 메시지의 전달 통로이자, 현장 질문·불만(물, 늦출일퇴 등)이 실시간으로 올라오는 양방향 채널이다.
...
## 19.6 Promoter → Promoter

같은 행사에서 함께 근무한 사이에만 열린다(Event context 유지 — Review 구조 무변경, worker_interview "서로 추천" 수용).
...
## Public Rating — 온도형 공개 지표로 추진

사용자가 참고할 수 있는 공개 평가. 당근 온도처럼 **하나의 지표로 쌓이고 오르내리는 형태**로 추진한다(worker_interview 수용 — 공개적으로 성장하는 지표가 등록·유지 유인).

- 입력: §19의 전 방향 평가(19.6 Promoter 간 평가 포함), 완료 이력
- 활용: §15 Leader Certification 취득 조건과 §13.7 강등의 입력 후보
- 표기 방식·산식·조작 방지(상호 품앗이 인플레이션)는 `[정책]`(§28)

예:

```text
4.8 / 5.0 → 온도형 표기(예: 38.2°) [정책]
```
...
- Platform Fee 구조
- 원천징수(3.3%) 처리 주체 — 플랫폼 지급대행 구조, 세전/세후 표시 방식(§9)
...
- In-App Chat 범위(행사 방 외 1:1)
- 개인번호 공개 여부
- 옵트인 연락 메모(인스타·연락처)의 노출 범위(§14.1)
