...
> Version 0.3 (2026-08-08) — D6 반영: P9 공고 탐색 신설, P4·P5 지원 트랙 확장, B1 간소화·B2 날짜 컨텍스트·B3/B4/B7 공개 모집 반영, 슬라이스 S6 추가
...
| [B1](screens/b1-buyer-home.md) | Buyer 홈 — 활성 행사·주의 항목 | Buyer | 데스크탑 | S2 | `/buyer` |
...
| [P4](screens/p4-requests.md) | 요청·지원 목록 | Worker | 모바일 | S3 | `/p/requests` |
| [P5](screens/p5-request-detail.md) | 요청 상세 — 수락/거절 (+공고 변형: 지원) | Worker | 모바일 | S3 | `/p/requests/[id]` · `/p/jobs/[roleId]` |
...
| [P9](screens/p9-job-board.md) | 공고 탐색 — 날짜별 | Worker | 모바일 | S6 | `/p/jobs` |
...
> **공개 랜딩(H0)**: 위 표 밖의 화면. 코드로 먼저 구현했고(`src/app/(marketing)/` — cipher.tv 무드의 시네마틱 다크 랜딩, 2026-08-08), 재현 스펙을 [H0 재현 프롬프트](screens/h0-landing.md)로 역추출해 뒀다(단독 입력용 — \_common.md 불필요). 기존 "공개 페이지 없음" 전제는 이 결정으로 변경됨. 카피·클립·인물은 `src/lib/landing-content.ts`에서 교체한다.
...
**§26 밖 확장 — 공개 모집·지원 (D6, 슬라이스 S6)**: 기획서 §26 이후의 하이브리드 확장 결정(2026-08-08)으로 추가된 커버리지. Buyer 공개 모집·지원자 승인 = [B3](토글) [B7](검토·승인), Promoter 공고 탐색·지원 = [P9] [P5 공고 변형] [P4 내 지원 탭].
