...
- **레이아웃**: ① 새 요청 — 결원 인원 선택(NoShow/Cancelled 대상), 사유 확인 → 요청 생성. 요청 생성 전 해당 Role의 잔여 **Standby 지원자 n명** 안내 배지(있을 때 — 이미 조건에 동의한 1차 재충원 후보. 승인 권한은 Buyer([B7]), 이 화면은 안내만, D6 결정 7) ② **진행 타임라인 카드**(요청별): 탐색 중(후보 n명에게 요청 발송, 경과 시간) → 매칭됨(대체자 카드: 사진·이름·도착 예상, "도착 시 체크인으로 완료됩니다") → **도착 완료**(Fulfilled) / 실패(후보 소진 — Buyer 통지됨) / 철회(수동, 또는 Buyer의 Standby 승인으로 결원 해소 시 자동 — Cancelled) ③ 재탐색 버튼(Matched 이탈 시).
...
- **정책**: Emergency Premium 부담 주체 표시 `[정책]`, 탐색 반경·시간 한도 `[정책]`, 긴급 후보 탐색에 해당 Role Standby 지원자 우선 포함 여부 `[정책]` — 기본 방향은 채널 분리: Standby 재충원은 Buyer 승인(일반 단가), Replacement 콜드콜은 Emergency Premium(D6 "같은 결원, 두 가격").
